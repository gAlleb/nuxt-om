      ...
      <div v-if="terraData" class="container">  
        <h2 class="my-5 text-2xl">Now playing:</h2>      
            ...
            <div class="ice-player-el" >
               <div class="relative">
                 <img class="shadow-np dark:shadow-np-dark rounded-xl h-auto w-32 shadow-2xl cursor-pointer sm:w-52" :src="terraData.np.now_playing.song.art" alt="Album Cover"  @click="openLightbox(terraData.np.now_playing.song.art, 0)" >
               ...
               <div class="ms-2 np-main">
                ...
                :style="dynamicBackgroundColor"> 
                  <span class="text-xl sm:text-2xl" :style="{color: dynamicTextColor.color}">{{ terraData.np.now_playing.song.title }}</span><br/>
                  <span class="text-lg sm:text-xl" :style="{color: dynamicTextColor.color}">{{ terraData.np.now_playing.song.artist }}</span><br/>
                  <span class="text-md sm:text-lg" :style="{color: dynamicTextColor.color}" style="font-family: monospace">{{ np_ac.isLoading ? '' : minSec(np_ac.progress['station:terra'].elapsed) }} / {{ np_ac.isLoading ? '' : minSec(np_ac.progress['station:terra'].duration) }}</span> 
                </div>
            ...
            <div class="absolute" style="height: 30px; transition: width 1s linear; opacity: 0.5" :style="{background: dynamicTextColor.color, width: `${( np_ac.progress['station:terra'].elapsed /  np_ac.progress['station:terra'].duration * 100).toFixed(2)}%` }"></div>
            </div>
...
const dynamicBackgroundColor = computed(() => {
  const color = np_ac.dominantColors['station:terra'];
