      ...
      <div v-if="comaData" class="container">  
        <h2 class="my-5 text-2xl">Now playing:</h2>      
            <div class="ice-player-el" >
               <div class="relative">
                 <img class="rounded-xl h-auto w-32 shadow-2xl cursor-pointer sm:w-52 shadow-np dark:shadow-np-dark" :src="comaData.np.now_playing.song.art" alt="Album Cover"  @click="openLightbox(comaData.np.now_playing.song.art, 0)" >
               ...
               <div class="ms-2 np-main">
                <div class="px-3 py-1 mb-0 sm:py-4 sm:mb-2 rounded-xl w-full text-muddy-waters-800 dark:text-muddy-waters-200  bg-opacity-50 ellipsify shadow-np dark:shadow-np-dark"
                :style="dynamicBackgroundColor"> 
                  <span class="text-xl sm:text-2xl" :style="{color: dynamicTextColor.color}">{{ comaData.np.now_playing.song.title }}</span><br/>
                  <span class="text-lg sm:text-xl" :style="{color: dynamicTextColor.color}">{{ comaData.np.now_playing.song.artist }}</span><br/>
                  <span class="text-md sm:text-lg":style="{color: dynamicTextColor.color}" style="font-family: monospace">{{ np_ac.isLoading ? '' : minSec(np_ac.progress['station:coma'].elapsed) }} / {{ np_ac.isLoading ? '' : minSec(np_ac.progress['station:coma'].duration) }}</span> 
                </div>
                <div class="mt-2 sm:mt-2 text-md sm:text-xl px-3 py-1 sm:py-5 rounded-xl w-full text-muddy-waters-800 dark:text-muddy-waters-200 ellipsify shadow-np dark:shadow-np-dark"
                :style="dynamicBackgroundColor"> 
                <span class="text-sm sm:text-lg" :style="{color: dynamicTextColor.color}"> 
                 Show: {{ comaData.np.now_playing.playlist }}
                </span>
                </div>
               </div>
            </div>
            ...
            :style="dynamicBackgroundColor">
            <div class="absolute" style="height:30px;  transition: width 1s linear" :style="{background: dynamicTextColor.color, width: `${( np_ac.progress['station:coma'].elapsed /  np_ac.progress['station:coma'].duration * 100).toFixed(2)}%` }"></div>
            </div>
          ...
          <div class="mt-2 rounded-xl  ice-player-el bg-sxvx-light dark:bg-sxvx-dark text-muddy-waters-300" >
              <div class=" relative">
                <img 
              v-if="comaData.np.playing_next.song.art" 
...
const dynamicBackgroundColor = computed(() => {
  const color = np_ac.dominantColors['station:coma'];
